...

{
    ...
    if(CANVAS instanceof HTMLCanvasElement) {
        ...
        const IMGS = Array.from(document.querySelectorAll('img.slider__item')) as HTMLImageElement[];
        const CTX = CANVAS.getContext('2d');

        async function waitForImagesToLoad(imgs : HTMLImageElement[]) : Promise<HTMLImageElement[]>
        {
            const PROMISES : Promise<HTMLImageElement>[] = imgs.map((img) => {
        ...
        waitForImagesToLoad(IMGS).then((loadedImgs) => {
            ...
            if(CTX !== null) {
                ...
                function sortImgPiecesByColumn(imgPieces : ImagePiece[]) : ImagePiece[] 
                {
